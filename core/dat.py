...
class DACorr(DAttentionBaseline):
        ...
        def __init__(self, q_size, 
            ...
            super().__init__(q_size, kv_size, n_heads, n_head_channels, n_groups,
                    attn_drop, proj_drop, stride, 
                    offset_range_factor, use_pe, dwc_pe,
                    no_off, fixed_pe, stage_idx)
        ...
        def forward(self, x, y):
            ...
            # return y, pos.reshape(B, self.n_groups, Hk, Wk, 2), reference.reshape(B, self.n_groups, Hk, Wk, 2)
            return attn.view(B, H, W, 1, H, W)

if __name__ == "__main__":
    fmap1 = torch.randn(1, 3, 26, 42)
    fmap2 = torch.randn(1, 3, 26, 42)
    fmap_size = fmap1.shape[-2:]
    dim = fmap1.shape[1]
    heads = 1
    hc = dim // heads
    DAC = DACorr(fmap_size, fmap_size, heads, hc)
