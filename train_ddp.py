...
def main(args):
    ...
    while logger.total_steps <= args.num_steps:
        train(model, train_loader, optimizer, scheduler, logger, scaler, gpu_id, args)
...
def train(model, train_loader, optimizer, scheduler, logger, scaler, gpu_id, args):
    for i_batch, data_blob in enumerate(train_loader):
        tic = time.time()
        # image1, image2, flow, valid = [x.cuda() for x in data_blob]
        image1, image2, flow, valid = [x.to(gpu_id) for x in data_blob]
        ...
        # Validate
        if gpu_id == 0 and logger.total_steps % args.val_freq == args.val_freq - 1:
